...
# def get_profile():
#     """
#     Fetch the logged-in user's details using their registered email.
#     The email should be provided as a query parameter.
#     """
#     email = request.args.get("email")
#     if not email:
#         return jsonify({"status": 0, "msg": "Email is required to fetch profile details.", "class": "error"}), 400
...
# def update_details():
#     """
#     Update user details based on email (unique identifier).
#     The email field cannot be updated.
#     """
#     input_data = request.get_json()
#     if not input_data or "email" not in input_data:
#         return jsonify({"status": 0, "msg": "Email is required to update details.", "class": "error"}), 400

#     email = input_data["email"]
#     # Remove email from update payload to prevent its modification.
...
#     result = db.useres.update_one({"email": email}, {"$set": update_data})
...
#     """
#     input_data = request.get_json()
#     if not input_data or not all(k in input_data for k in ("email", "old_password", "new_password")):
#         return jsonify({"status": 0, "msg": "Email, old_password, and new_password are required.", "class": "error"}), 400
...
#     db.useres.update_one({"email": email}, {"$set": {"password": hashed_new_password}})
    
#     return jsonify({"status": 200, "msg": "Password updated successfully."})

# @user_bp.route("/getlistuser", methods=["GET"])
# def get_list_user():
#     """
#     Admin endpoint to search for a user by email.
#     The email should be provided as a query parameter.
#     """
#     email = request.args.get("email")
#     if not email:
...
#     # Remove sensitive information such as password.
#     user.pop("password", None)
#     return jsonify({"status": 200, "msg": "User found.","user": user})
